...
const ModalGrupos = ({ abrirConsulta, abrirRegistro, onCloseProp, objConsulta }) => {
    ...
    const ValidarObjGrupo = () => {
        ...
        if(!CamposVacios(grupo)){
            if(!idPrograma || !idPrograma.toString().trim() || !SoloNumeros(idPrograma)){
                alert("Selección incorrecta de programa académico!");
                setPrograma({});
            }
            else if(!ficha.toString().trim() || !HastaCincuenta(ficha) || !SoloNumeros(ficha)){
                alert("Número de ficha incorrecto!");
                setFicha('');
            }else if(!codigoGrupo.toString().trim() || !HastaCien(codigoGrupo) || !AlfaNumericaSinEspacio(codigoGrupo)){
